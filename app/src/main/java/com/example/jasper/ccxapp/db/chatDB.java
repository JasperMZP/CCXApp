...
import com.example.jasper.ccxapp.entities.Chat;
...
//    }
    ...
    //添加新聊天室
    public static void addnewchatroom(String userName, String chatroom, ArrayList<String> otherusers, final userBackListener userBackListener) {
        List<BmobObject> friends = new ArrayList<BmobObject>();
        ArrayList<Chat> chats = getChats(userName, chatroom, otherusers);
        for (int i = 0; i < otherusers.size(); i++) {
            friends.add(chats.get(i));
        }
        new BmobBatch().insertBatch(friends).doBatch(new QueryListListener<BatchResult>() {
            ...
            public void done(List<BatchResult> list, BmobException e) {
                if(e==null){
                    ...
                    Log.e("chat", "添加新群聊失败："+e.getMessage());
                    userBackListener.showResult(false, e.getMessage());
                }
            }
        });
...
//    }
    ...
    //构建Chat类
    private static ArrayList<Chat> getChats(String userName1, String chatroom, ArrayList<String> otheruser) {
        ArrayList<Chat> chats = new ArrayList<Chat>();
        for(int i = 0; i < otheruser.size(); i++){
            Chat achat = new Chat();
            achat.setUserName(userName1);
            achat.setChatroomName(chatroom);
            achat.setOtherUser(otheruser.get(i));
            chats.add(achat);
        }
        return chats;
    }
